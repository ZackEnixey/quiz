...
const QuestionsSlide: FC<IQuestionsSlide> = (props) => {
    ...
    const initialCurrentQuestion = questionObject?.length > 0 ? questionObject[counter] : undefined;
    const [currentQuestion, setCurrentQuestion] = useState<any>(initialCurrentQuestion);
                ...
                <div className="quiz-box">
                    ...
                    <div className="answers">
                        {currentQuestion.answer.map((answer: string, index: number) => (
                            <div key={index} onClick={() => onAnswerSelect(index)}> {answer} </div> 
                        ))}
                    </div>
                </div>
            );
            ...
            return (
                <div className="quiz-box">
                    ...
                    <div className="answer">
                        <div className={`blurred ${toggleAnswer ? "" : "blurred_text"} `} onClick={showTheAnswer}>{currentQuestion.answer}</div>
                    </div>
                </div>
            );
        }
    ...
    return(
        <div>
            ...
            <div className="buttons-container">
                {(counter) > 0 && (
                    <button className="quiz-button quiz-button-back" onClick={() => goToNextQuestion(false)}>
                    Back
                    </button>
                )}
                {(counter < questionObject.length) && (
                    <button className="quiz-button quiz-button-next" onClick={() => goToNextQuestion(true)}>
                    Next
                    </button>
                )}
                </div>
